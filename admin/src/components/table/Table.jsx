...
export const List = () => {
  const { data, loading, error } = useFetch("http://localhost:8000/api/admin/orders/latest");
  // const [rows, setData] = useState(data);
  const rows = data.map(order => {
    const orderFormated = JSON.parse(JSON.stringify(order));
    orderFormated.products = order.products.map(product => product.laptopName).join(',');
